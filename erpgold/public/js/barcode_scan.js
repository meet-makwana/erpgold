frappe.provide("erpnext.stock"); frappe.provide("erpnext.utils.BarcodeScanner");
// frappe.provide("erpgold.erpgold.barcode")
...
erpnext.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		...
		this.custom_metal_type_field = opts.custom_metal_type_field || "custom_metal_type";
		this.custom_purity_field = opts.custom_purity_field || "custom_purity";
	...
	set_item(row, item_code, barcode, batch_no, serial_no , custom_metal_type, custom_purity) {
		return new Promise(resolve => {
			const increment = async (value = 1) => {
				...
				item_data[this.custom_purity_field] = custom_purity;
			...
			if (this.prompt_qty) {
				...
				this.prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no , custom_metal_type, custom_purity);
			} else {
